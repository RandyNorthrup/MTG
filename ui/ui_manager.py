import os
# UPDATED imports (add QDialog, QMouseEvent, QPaintEvent, QDrag-related types used / referenced)
...
try:
    from shiboken6 import isValid as _q_is_valid  # preferred
except Exception:
    def _q_is_valid(o):
        if o is None:
            return False
        try:
            # Access a harmless Qt method; deleted objects raise RuntimeError
            o.objectName()
            return True
        except Exception:
            return False
...
class PlayArea(QWidget):
    ...
    # NEW: minimal API expected externally
    def refresh_board(self):
        self.update()

    def enable_drag_and_drop(self):
        # placeholder (custom painting version already handles mouse events)
        pass
