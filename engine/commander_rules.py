...
class CommanderTracker:
    # key by the card's string id (UUID), not int
    cast_counts: Dict[str, int] = field(default_factory=dict)
    # commander combat damage tracked by (defender_pid, commander_owner_pid)
    damage: Dict[Tuple[int, int], int] = field(default_factory=dict)

    def tax_for(self, commander_card_id: str) -> int:
        """+2 generic per previous cast of THIS commander from the command zone (CR 903.8)."""
